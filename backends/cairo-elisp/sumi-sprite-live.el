...
;;
;; Like sumi-sprite.el but it watches sumi-sprite.bin and re-applies each new
;; frame to PERSISTENT buffers (allocated once), so a running game streamed
;; through json-to-bin-bridge renders live and natively.  load-image / screen
;; only run when the buffer is empty (bufsurf[id]==0), so sprite PNGs decode once
;; even though the bridge re-sends the setup every frame; per-frame draws (clear,
;; blits) re-run each tick.  Buffer 0 (the screen) is shown in the window.
;;
...
;;      48 area 56 bufsurf[256] 64 bufcr[256] 72 cur_cr 80 screen_surf
(seq
 ...
 ;; Apply one frame's commands to the persistent buffers.
